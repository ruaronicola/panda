...
#if 0
void testaf(Shad *shad, llvm::Instruction *I, uint64_t slot1, uint64_t slot2)
{
  printf("TESTAF\n");
  // Call after_tainted_branch, pass return back to branchFCon
  char* res = after_tainted_branch(shad, I, slot1, slot2);
  printf("CALLED\n");
  branchFCon(res);
}
#endif

//char* after_tainted_branch(Shad *shad, llvm::Instruction *I, uint64_t slot1, uint64_t slot2)
